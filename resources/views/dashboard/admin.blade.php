...
@section('title', 'Admin — MedBooking')

@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-10">
        <div class="mb-8">
            <h1 class="text-2xl lg:text-3xl font-bold text-gray-900">Admin</h1>
            <p class="mt-2 text-gray-600">Overview of doctors, services, and schedule. CRUD is available via the API.</p>
        </div>
                ...
                <div class="space-y-3 max-h-[360px] overflow-y-auto">
                    @foreach ($services as $service)
                        <div class="rounded-xl border border-gray-200 bg-gray-50/50 p-4">
                            ...
                            <div class="text-sm text-gray-500 mt-0.5">{{ $service->duration_minutes }} min</div>
                            <span class="inline-flex mt-2 rounded-lg bg-[#6B21A8]/10 px-2 py-0.5 text-xs font-medium text-[#6B21A8]">Appointments: {{ $service->appointments_count ?? 0 }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                <h2 class="text-lg font-semibold text-gray-900 mb-4">Schedule (appointments)</h2>
                <div class="space-y-3 max-h-[360px] overflow-y-auto">
