...
@section('title', 'Register — MedBooking')

@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        <div class="max-w-[400px] mx-auto">
            <div class="text-center mb-6">
                <h1 class="text-2xl font-bold text-gray-900">Client registration</h1>
                <p class="mt-2 text-gray-500 text-sm">Role <span class="text-[#6B21A8] font-medium">client</span>. Admin and doctor use demo accounts.</p>
            </div>
            @if(auth()->check() && ! auth()->user()->isClient())
                <div class="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-left text-sm text-amber-950">
                    You are signed in as <strong>{{ auth()->user()->role }}</strong>. This form creates a <strong>client</strong> account to link a guest booking. After submitting you will be logged in as the new user.
                </div>
            @endif
            @if($errors->has('email') && old('email'))
                <div class="mb-4 rounded-xl border border-[#6B21A8]/25 bg-[#6B21A8]/5 px-4 py-3 text-center text-sm text-gray-800">
                    <a href="{{ route('login', ['email' => old('email')]) }}" class="font-medium text-[#6B21A8] hover:underline">Log in with this email</a>
                    — your guest booking will link after sign-in.
                </div>
            @endif
            <div class="rounded-2xl border border-gray-200 bg-white p-6 sm:p-8 shadow-sm">
                <form method="POST" action="{{ url('/register') }}" class="space-y-5">
                    ...
                    <div class="flex items-center justify-between text-sm pt-1">
                        ...
                        <a href="{{ route('login') }}" class="font-medium text-[#6B21A8] hover:text-[#5B1B8A]">Log in</a>
                    </div>
