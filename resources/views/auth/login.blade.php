...
@section('title', 'Log in — MedBooking')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        <div class="max-w-[400px] mx-auto">
            <div class="text-center mb-6">
                ...
                <p class="mt-2 text-gray-500 text-sm">Demo: client@example.com, admin@example.com, doctor1@example.com · password <code class="bg-gray-100 px-1.5 py-0.5 rounded text-gray-700">password</code></p>
            </div>
            @if(auth()->check() && ! auth()->user()->isClient())
                <div class="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-left text-sm text-amber-950">
                    You are signed in as <strong>{{ auth()->user()->role }}</strong>. Enter <strong>client</strong> credentials to switch to that account, or
                    <form method="POST" action="{{ route('logout') }}" class="inline">
                        @csrf
                        <button type="submit" class="font-medium text-[#6B21A8] underline bg-transparent border-0 p-0 cursor-pointer hover:text-[#5B1B8A]">log out</button>
                    </form>
                    first.
                </div>
            @endif
            <div class="rounded-2xl border border-gray-200 bg-white p-6 sm:p-8 shadow-sm">
                <form method="POST" action="{{ url('/login') }}" class="space-y-5">
                    ...
                    <div class="flex items-center justify-between text-sm pt-1">
                        ...
                        <a href="{{ route('register') }}" class="font-medium text-[#6B21A8] hover:text-[#5B1B8A]">No account? Register</a>
                    </div>
