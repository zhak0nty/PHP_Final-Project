...
@section('title', 'Appointment created — MedBooking')
        ...
        <div class="max-w-md mx-auto">
            <div class="rounded-3xl border border-gray-200 bg-white p-8 shadow-[0_4px_24px_rgba(0,0,0,0.06)] text-center">
                ...
                <p class="mt-3 text-gray-600">
                    We notified the doctor. Wait for confirmation if needed.
                </p>
                @if (!empty($appointment['datetime']) || !empty($appointment['doctor']) || !empty($appointment['service']))
                    <div class="mt-6 rounded-2xl border border-gray-200 bg-gray-50/80 px-5 py-4 text-left text-sm">
                        @if (!empty($appointment['datetime']))
                            <div class="flex justify-between gap-2"><span class="text-gray-500">Date & time</span><span class="font-medium text-gray-900">{{ $appointment['datetime'] }}</span></div>
                        @endif
                        @if (!empty($appointment['doctor']))
                            <div class="flex justify-between gap-2 mt-2"><span class="text-gray-500">Doctor</span><span class="font-medium text-gray-900">{{ $appointment['doctor'] }}</span></div>
                        @endif
                        @if (!empty($appointment['service']))
                            <div class="flex justify-between gap-2 mt-2"><span class="text-gray-500">Service</span><span class="font-medium text-gray-900">{{ $appointment['service'] }}</span></div>
                        @endif
                    </div>
                @endif
                @if (!empty($guest) && $guest)
                    <p class="mt-4 text-sm text-gray-600">
                        Register with this email within 5 minutes — the appointment will appear in your account.
                    </p>
